...
这个版本已经比最初更完整了，新增了这些功能：

- 可以直接用 **GUI 图形界面**，不一定非要敲命令
- 一个 Excel 里如果有 **多个子表格**，会自动依次出图
- 同一个 sheet 里如果有 **多个块状子表格**，也能自动拆开处理
- 如果信息表里带有原始子表路径/文件名，会自动按原子表名字命名输出
- GUI 里可以直接选择常用 `colormap`，还能看到色卡预览
...
- 如果一个 Excel 里有多个子表格，也能按顺序分别画图
- 如果信息表里带有原始子表文件名，还会自动按原子表名字命名输出图片
...
## 4.1 如果你更想“点按钮”操作：可以直接用 GUI

如果你不想看命令行提示，也不想自己输很多路径，可以直接打开图形界面：
...
- 填写循环编号
- 设置标题、格式、DPI 等参数
- 直接选择常用 colormap，并看到色卡预览
- 点击“开始绘图”

如果 Excel 里有多个子表格，GUI 版也会自动依次输出多张图，不需要你手工拆文件。

GUI 下面还有“运行日志”区域，你可以直接看到程序正在处理哪个子表格、最终保存到了哪里。
...
- 你可以改成别的文件名
- 也可以改成 `svg` 或 `pdf`
- 如果 Excel 里只有 1 个可绘制子表格，就按你写的文件名保存
- 如果 Excel 里有多个子表格，程序会把这个路径当作“输出目录 + 默认示例名”，然后按各子表名字分别保存多张图
...
如果你的 Excel 里实际上合并了多个子表格，也不用手工拆文件。
...
```text
D:\PythonProject\cycle\cyclingtest_2026.3.14-Gr_only_NMC0.788mg-CritCRate_20260314155506_DefaultGroup_37_1.png
D:\PythonProject\cycle\cyclingtest_2026.3.14-Gr_only_NMC0.799mg-CritCRate_20260314160000_DefaultGroup_37_2.png
```

也就是：**目录按你输入的走，文件名按识别到的子表名字自动生成。**
...
- **统一颜色**：所有曲线都用同一种颜色
- **colormap**：不同循环自动使用一组颜色方案

例如：

- 统一颜色填 `#1f77b4`：所有线都偏蓝色
- colormap 选 `tab10`：不同循环会自动分配不同颜色
- colormap 选 `viridis`：不同循环会按渐变色分配

简单记法：

- 勾选“按循环着色” → 主要看 `colormap`
- 不勾选“按循环着色” → 主要看“统一颜色”
...
```text
D:\你的输出目录\cyclingtest_2026.3.14-Gr_only_NMC0.788mg-CritCRate_20260314155506_DefaultGroup_37_1.png
```

如果没有识别到信息表中的原始子表信息，则会退回到类似：

```text
MixedSheet_part1.png
MixedSheet_part2.png
```
...
### 问题 6：GUI 里 colormap 和统一颜色有什么区别？

简单理解：

- `colormap`：给不同循环分配不同颜色
- `统一颜色`：全部曲线统一一种颜色

如果你已经勾选“按循环着色”，那更应该关注 `colormap`。

### 问题 7：为什么我输入的是一个文件名，最后却生成了多张图？

这是因为程序识别到你的 Excel 里实际包含多个可绘制子表格。
...
如果你以后想让别人帮你维护，或者你自己想继续改功能，可以先知道这些文件是干什么的：

```text
main.py          程序主入口（交互式 / 命令行 / --gui）
gui.py           图形界面
config.py        默认参数
utils.py         输入解析、路径处理
parser.py        表头识别、列名匹配、模式分类
data_loader.py   多子表格读取、命名匹配、数据整理
plotter.py       实际绘图与保存
README.md        项目说明
USER_MANUAL.md   这份用户手册
```
